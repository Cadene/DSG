...
parser.add_option("-n",dest="n_round",type="int",help="epoch number", default=1)
...
if(options.cv>0 ):
    ...
    if(options.xtest != None):
        Xtest = pd.read_csv(options.xtest, index_col=0).fillna(value=0,inplace=True);
	print(Xtest.shape)
        ...
        if(os.path.isdir(options.save)):
            int_key = np.random.randint(1,10000)
            date_key = os.times()[4] * 10
            options.save = os.path.join(options.save,"pred_"+str(int_key)+"_"+str(date_key)+".csv")
