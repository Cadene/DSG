...
def add_final_features(X, dfKey, cols):
    ...
    moyenneByCustomer = X.groupby(dfKey.CustomerMD5Key).mean()
    ecartByCustomer = X.groupby(dfKey.CustomerMD5Key).std()
    ecartByCustomer = ecartByCustomer.fillna(0)

    for col in cols:
        new_features['mean_customer_'+col] = dfKey.CustomerMD5Key.apply(lambda x : moyenneByCustomer[col][x])
        new_features['std_customer_'+col] = dfKey.CustomerMD5Key.apply(lambda x : ecartByCustomer[col][x])
        new_features['mean_broker_'+col] = dfKey.SCID.apply(lambda x : moyenneBySCID[col][x] if x == x else 0)
        new_features['std_broker_'+col] = dfKey.SCID.apply(lambda x : ecartBySCID[col][x] if x == x else 0)
...
moyenneBySCID = X_full3.groupby(X_full2.SCID).mean()
ecartBySCID = X_full3.groupby(X_full2.SCID).std()
...
print(np.setdiff1d(X_train3.columns, X_test4.columns))
if hasPrivate:
    print(np.setdiff1d(X_train3.columns, X_private4.columns))
...
if False:
    X_train4.to_csv('data/interim/X_train_features_3.csv')
    X_test4.to_csv('data/interim/X_test_features_3.csv')
    if hasPrivate:
        X_private4.to_csv('data/interim/X_private_features_3.csv')
