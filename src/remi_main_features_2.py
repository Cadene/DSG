...
X_full = pd.concat([X_train2, X_test2])
#X_full = pd.concat([X_train2, X_test2, X_private2])
centrer_reduire(X_full, X_train2, X_test2, cols_continuous)

# globals for train features
meanConvertedByBroker = X_train.groupby(X_train.SCID).mean().Converted
stdConvertedByBroker  = X_train.groupby(X_train.SCID).sum().Converted

# globals for global features
countNbQuotesBySCID = X_full.groupby('SCID').count().CustomerMD5Key
countNbUsersBySCID = X_full.groupby('SCID').CustomerMD5Key.nunique()
countSocioDemographicId = X_full.groupby('SocioDemographicId').count().CustomerMD5Key
countAffinityCodeId = X.groupby('AffinityCodeId').count().CustomerMD5Key    

print('brefore X_train3', int((time.time() - t_start) * 1000))
